...
func NewGridStore(ctx context.Context, db *DB, table string, logger *slog.Logger) (*GridStore, error) {
	...
	if err := m.Migrate(ctx, "timescaledb_grid_"+table, gridMigrations(db.db, table)); err != nil {
		return nil, fmt.Errorf("timescaledb grid migration: %w", err)
	}
	return &GridStore{db: db, table: table, logger: logger}, nil
}

// StoreGridTelegram inserts a grid telegram into TimescaleDB.
func (s *GridStore) StoreGridTelegram(ctx context.Context, t domain.GridTelegram) error {
	...
	if err != nil {
		s.logger.ErrorContext(
			ctx, "timescaledb: StoreGridTelegram failed",
