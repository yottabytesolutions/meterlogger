...
func solarMigrations(db *sql.DB, table string) []schemastore.Migration {
    ...
    peak_watts      INT
				...
				_, err = db.ExecContext(
					ctx,
					fmt.Sprintf(`SELECT create_hypertable('%s_inverters', 'ts', if_not_exists => TRUE)`, table),
				)
				return err
			},
		},
	...
	return []schemastore.Migration{
		{
			...
			Up: func(ctx context.Context) error {
				...
				ddlAndHypertable := []struct {
					ddl       string
					tableExpr string
				}{
					{ducoBoxGeneralTableSQL(base), base + "_box_general"},
					{ducoNodeTableSQL(base), base + "_node"},
					{ducoBoxNodeTableSQL(base), base + "_box_node"},
					{ducoValveTableSQL(base), base + "_valve"},
				}
				for _, item := range ddlAndHypertable {
					if _, err := db.ExecContext(ctx, item.ddl); err != nil {
						return err
					}
					if _, err := db.ExecContext(
						ctx,
						fmt.Sprintf(`SELECT create_hypertable('%s', 'ts', if_not_exists => TRUE)`, item.tableExpr),
					); err != nil {
						return err
					}
				}
