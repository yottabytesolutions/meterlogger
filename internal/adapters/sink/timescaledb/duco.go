...
func NewDucoStore(ctx context.Context, db *DB, base string, logger *slog.Logger) (*DucoStore, error) {
	...
	if err := m.Migrate(ctx, "timescaledb_duco_"+base, ducoMigrations(db.db, base)); err != nil {
		return nil, fmt.Errorf("timescaledb duco migration: %w", err)
	}
	return &DucoStore{db: db, base: base, logger: logger}, nil
}

// StoreBoxStatus inserts duco box status into TimescaleDB.
func (s *DucoStore) StoreBoxStatus(ctx context.Context, b domain.DucoBoxStatus) error {
	...
	if err != nil {
		s.logger.ErrorContext(ctx, "timescaledb: StoreBoxStatus failed", slog.Any("error", err))
	}
	return err
}

// StoreNodeData inserts duco node data into the appropriate TimescaleDB table.
func (s *DucoStore) StoreNodeData(ctx context.Context, nodeData domain.DucoNodeStatus) error {
	...
	default:
		s.logger.WarnContext(
			ctx, "timescaledb: unknown node type, skipping",
...
func (s *DucoStore) storeRFSensor(ctx context.Context, now time.Time, d domain.DucoRFSensorStatus) error {
	...
	if err != nil {
		s.logger.ErrorContext(ctx, "timescaledb: store RF sensor node failed", slog.Any("error", err))
	}
	return err
}
...
func (s *DucoStore) storeBoxNode(ctx context.Context, now time.Time, d domain.DucoNodeBoxStatus) error {
	...
	if err != nil {
		s.logger.ErrorContext(ctx, "timescaledb: store box node failed", slog.Any("error", err))
	}
	return err
}
...
func (s *DucoStore) storeValveNode(ctx context.Context, now time.Time, d domain.DucoNodeBoxValveStatus) error {
	...
	if err != nil {
		s.logger.ErrorContext(ctx, "timescaledb: store valve node failed", slog.Any("error", err))
	}
	return err
}
