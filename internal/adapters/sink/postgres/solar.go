...
// SolarStore implements domain.EnvoySolarRepository for PostgreSQL.
...
func NewSolarStore(ctx context.Context, db *DB, table string, logger *slog.Logger) (*SolarStore, error) {
	...
	if err := m.Migrate(ctx, "postgres_solar_"+table, solarMigrations(db.db, table)); err != nil {
		return nil, fmt.Errorf("postgres solar migration: %w", err)
	}
	return &SolarStore{db: db, table: table, logger: logger}, nil
}

// StoreEnvoySolarData inserts solar data and inverter details into PostgreSQL.
func (s *SolarStore) StoreEnvoySolarData(ctx context.Context, d domain.EnvoySolarData) error {
	...
	if err != nil {
		s.logger.ErrorContext(ctx, "postgres: store solar failed",
	...
	for _, inv := range d.Inverters {
		...
		if invErr != nil {
			s.logger.ErrorContext(ctx, "postgres: store inverter failed", slog.Any("error", invErr))
