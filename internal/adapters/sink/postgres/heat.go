...
func NewHeatStore(ctx context.Context, db *DB, table string, logger *slog.Logger) (*HeatStore, error) {
	...
	if err := m.Migrate(ctx, "postgres_heat_"+table, heatMigrations(db.db, table)); err != nil {
		return nil, fmt.Errorf("postgres heat migration: %w", err)
	}
	return &HeatStore{db: db, table: table, logger: logger}, nil
}

// StoreHeatTelegram inserts a heat telegram into PostgreSQL.
func (s *HeatStore) StoreHeatTelegram(ctx context.Context, t domain.HeatTelegram) error {
	...
	if err != nil {
		s.logger.ErrorContext(ctx, "postgres: StoreHeatTelegram failed",
