...
func testDB(t *testing.T) (*postgres.DB, sqlmock.Sqlmock) {
	t.Helper()
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("sqlmock: %v", err)
	}
	return postgres.NewDBFromSQL(db, panicLogger()), mock
}
...
func TestHeatStore_StoreHeatTelegram(t *testing.T) {
	...
	store, storeErr := postgres.NewHeatStore(context.Background(), db, "heat", panicLogger())
	if storeErr != nil {
