...
// the PostgreSQL dialect and DSN handling.
package postgres
...
// Config holds the connection parameters for a PostgreSQL sink.
type Config struct {
	Host     string
	Port     int
	User     string
...
// HeatStore implements domain.HeatMeterRepository for PostgreSQL.
...
func New(ctx context.Context, cfg Config, logger *slog.Logger) (*DB, error) {
	return sqlsink.Open(ctx, sqlsink.PostgresDialect(), buildDSN(cfg), cfg.Host, cfg.Database, logger)
}
...
func NewDBFromSQL(db *sql.DB, logger *slog.Logger) *DB {
	return sqlsink.NewDBFromSQL(sqlsink.PostgresDialect(), db, logger)
}
