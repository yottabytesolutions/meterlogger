...
func expectDucoMigrationApplied(mock sqlmock.Sqlmock) {
	expectMigrationAlreadyApplied(mock, "postgres_duco_duco")
}

func newDucoStore(t *testing.T) (*postgres.DucoStore, sqlmock.Sqlmock) {
	t.Helper()
	db, mock := testDB(t)
	expectDucoMigrationApplied(mock)
	store, storeErr := postgres.NewDucoStore(context.Background(), db, "duco", panicLogger())
	if storeErr != nil {
...
func TestDucoStore_StoreNodeData_ValveNode(t *testing.T) {
	store, mock := newDucoStore(t)
	...
	if writeErr := store.StoreNodeData(context.Background(), domain.DucoNodeBoxValveStatus{}); writeErr != nil {
		t.Errorf("StoreNodeData ValveNode: %v", writeErr)
	}
	if expectErr := mock.ExpectationsWereMet(); expectErr != nil {
		t.Error(expectErr)
	}
