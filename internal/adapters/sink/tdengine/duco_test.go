...
func expectDucoMigrationApplied(mock sqlmock.Sqlmock) {
	expectMigrationAlreadyApplied(mock, "tdengine_duco_duco")
}

func newDucoStore(t *testing.T) (*tdengine.DucoStore, sqlmock.Sqlmock) {
	t.Helper()
	db, mock := testDB(t)
	expectDucoMigrationApplied(mock)
	store, storeErr := tdengine.NewDucoStore(context.Background(), db, "duco", testLogger())
	if storeErr != nil {
