...
func NewHeatStore(ctx context.Context, db *DB, table string, logger *slog.Logger) (*HeatStore, error) {
	...
	if err := m.Migrate(ctx, "tdengine_heat_"+table, heatMigrations(db.db, table)); err != nil {
		return nil, fmt.Errorf("tdengine heat migration: %w", err)
	}
	return &HeatStore{db: db, table: table, logger: logger}, nil
}

// StoreHeatTelegram inserts a heat telegram into TDEngine.
func (s *HeatStore) StoreHeatTelegram(ctx context.Context, t domain.HeatTelegram) error {
	...
	_, err := s.db.db.ExecContext(
		ctx,
		fmt.Sprintf(
			`INSERT INTO %s (ts, meter_id, serial_no, power_w, energy_gj, t_forward_c, t_return_c, t_diff_c,`+
				` volume_cm3, seconds, max_flow, max_power_w) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			s.table,
		),
		t.Timestamp,
		t.MeterID,
	...
	if err != nil {
		s.logger.ErrorContext(
			ctx, "tdengine: StoreHeatTelegram failed",
