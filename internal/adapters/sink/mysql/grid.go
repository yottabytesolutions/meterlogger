...
func NewGridStore(ctx context.Context, db *DB, table string, logger *slog.Logger) (*GridStore, error) {
	...
	if err := m.Migrate(ctx, "mysql_grid_"+table, gridMigrations(db.db, table)); err != nil {
		return nil, fmt.Errorf("mysql grid migration: %w", err)
	}
	return &GridStore{db: db, table: table, logger: logger}, nil
}

// StoreGridTelegram inserts a grid telegram into MySQL.
func (s *GridStore) StoreGridTelegram(ctx context.Context, t domain.GridTelegram) error {
	...
	if err != nil {
		s.logger.ErrorContext(ctx, "mysql: StoreGridTelegram failed",
