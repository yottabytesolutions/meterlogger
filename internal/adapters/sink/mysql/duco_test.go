...
func expectDucoMigrationApplied(mock sqlmock.Sqlmock) {
	expectMigrationAlreadyApplied(mock, "mysql_duco_duco")
}

func newDucoStore(t *testing.T) (*mysql.DucoStore, sqlmock.Sqlmock) {
	t.Helper()
	db, mock := testDB(t)
	expectDucoMigrationApplied(mock)
	store, storeErr := mysql.NewDucoStore(context.Background(), db, "duco", panicLogger())
	if storeErr != nil {
